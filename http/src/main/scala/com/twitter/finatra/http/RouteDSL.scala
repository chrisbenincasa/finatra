...
private class FilteredDSL[FilterType <: HttpFilter : Manifest] extends RouteDSL {
  override private[http] val context = {
    val curr = contextRef()
    val nextCtx = curr.copy(buildFilter = getBuildFilterFunc(curr.buildFilter), name = s"filter")
    nextCtx
  }
  ...
  override def contextWrapper[T](f: => T): T = withContext(context)(f)
}

trait MutableRouteState {
  private[http] val context: RouteContext = RouteContext("", (_) => Filter.identity, "base")
...
private class PrefixedDSL(prefix: String) extends RouteDSL {
  override private[http] val context = {
    val curr = contextRef()
    curr.copy(prefix = curr.prefix + prefix, name = "prefix")
  }
...
private[http] case class RouteContext(prefix: String, buildFilter: (Injector) => HttpFilter, name: String)
...
private[http] trait RouteDSL extends MutableRouteState { self =>
  ...
  private[http] def add[RequestType: Manifest, ResponseType: Manifest](
    method: Method,
    route: String,
    name: String,
    admin: Boolean,
  ...
  private[http] def withContext[T](ctx: RouteContext)(f: => T): T = {
    val orig = contextRef()
    contextRef() = ctx
    val res = try f finally contextRef() = orig
    res
  }
}
